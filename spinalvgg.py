'''
#Adapted from @dipuk0506/SpinalNet/blob/master/MNIST_VGG/EMNIST_letters_VGG_and%20_SpinalVGG.py
'''
...
class SpinalVGG(nn.Module):
    ...
    def get_train_output(self, batch):
        outputs         = self.forward(batch['input'])
        _, prediction   = torch.max(outputs.data, 1)

        return {'output':outputs, 'prediction':prediction}
